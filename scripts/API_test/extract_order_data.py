...
# 文件路径设置
md_file_path = "d:\Gtihub\litemall_Test_Plan\litemall_Test_Plan\测试用文档\测试用例\API\接口测试用例_订单管理模块.md"
output_dir = "d:\Gtihub\litemall_Test_Plan\litemall_Test_Plan\date\ApiFox\Order"
output_csv_path = os.path.join(output_dir, "submit_order_params.csv")
        ...
        if len(parts) >= 12:  # 确保有足够的列（包含预期返回数据列）
            ...
            try:
                ...
                if '{' in request_params and '}' in request_params:
                    ...
                    user_coupon_id = params.get('userCouponId', '')
                    groupon_link_id = params.get('grouponLinkId', '')
                    groupon_rules_id = params.get('grouponRulesId', '')
            ...
            # 添加到提取的数据列表
            extracted_data.append([address_id, cart_id, coupon_id, user_coupon_id, groupon_link_id, groupon_rules_id, message, errno, errmsg])
...
if extracted_data:
    with open(output_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        # 写入表头，包含所有需要的字段
        writer.writerow(['addressId', 'cartId', 'couponId', 'userCouponId', 'grouponLinkId', 'grouponRulesId', 'message', 'errno', 'errmsg'])
        # 写入数据
