...
# 文件路径设置
md_file_path = "d:\Gtihub\litemall_Test_Plan\litemall_Test_Plan\测试用文档\测试用例\API\接口测试用例_购物车模块.md"
output_dir = "d:\Gtihub\litemall_Test_Plan\litemall_Test_Plan\date\ApiFox\Cart"
output_csv_path = os.path.join(output_dir, "add_to_cart_params.csv")
...
for i, line in enumerate(content):
    # 标记加入购物车测试用例表格的开始
    if '### 1.2 加入购物车测试用例' in line:
        # 跳过表头和分隔线
        header_line = i + 3  # 第i+1行是表格标题，i+2行是表头，i+3行是分隔线
        start_processing = True
        continue
    # 标记下一个测试用例表格的开始，停止处理
    elif start_processing and '### 1.' in line and not '### 1.2' in line:
        break
    ...
    if start_processing and '|' in line and len(line.strip()) > 0:
        ...
        if len(parts) >= 12:  # 确保有足够的列（包含预期返回数据列）
            ...
            # 添加到提取的数据列表
            extracted_data.append([goods_id, number, product_id, errno, errmsg])

# 写入CSV文件
if extracted_data:
    with open(output_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        # 写入表头，添加errno和errmsg字段
        writer.writerow(['goodsId', 'number', 'productId', 'errno', 'errmsg'])
        # 写入数据
